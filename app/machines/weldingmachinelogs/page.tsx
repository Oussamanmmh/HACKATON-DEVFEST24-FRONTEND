...
const Page = () => {
  ...
  const rows = machine.slice(0, 10).map((entry) => ({
    temperature: entry?.sensorData?.weld_temperature + "°C",
    powerconsumption: entry?.sensorData?.power_consumption + "KwH",
    vibrationlevel: entry?.sensorData?.vibration_level + "mm/s",
    armposition:
      entry?.sensorData?.arm_position?.x +
      ";" +
      entry?.sensorData?.arm_position?.y +
      ";" +
      entry?.sensorData?.arm_position?.z,
  ...
  const handleDownloadJSON = () => {
    ...
    link.download = "welding_robot_006_report.json"; // Set the filename
    link.click();
  };
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.writeFile(workbook, "welding_robot_006_report.xlsx"); // Set the filename
  };
  ...
  return (
    <div className="bg-[#F5F6FA] p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-10">
        ...
        <div>
          <p className="text-3xl font-bold mb-4">Welding Robots</p>
          ...
          <p className="text-lg mb-4">
            ...
            welds in the car assembly process.
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
        <Link href="/machines/weldingmachinelogs/weldingmachine">
            ...
            <TableBody>
              {rows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell align="right">{row.powerconsumption}</TableCell>
